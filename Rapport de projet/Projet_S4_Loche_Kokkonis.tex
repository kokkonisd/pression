...
% Chaque pieds applique au niveau du sol une fraction de la force totale.
...
%La fraction de la force vue au niveau des pieds change donc en fonction de l'assise. 

%Ces fractions changent car la position du point d'application de la force totale sur le plateau de la chaise change en fonction de l'assise. En fait, il est lié à la position du centre de masse projeté sur la chaise. 

%Vous pouvez voir sur la figure \ref{fig:illustr_chaise_forces} une illustration de la proposition faite précédemment.
...
%On ne connais donc pas l'emplacement de la force totale mais on cherche à la déterminer à partir des valeurs des forces mesurées au niveau des pieds.
...
\draw (A1) node[below right] {$A_1$} node{$\bullet$};
\draw (A2) node[above right] {$A_2$} node{$\bullet$};
\draw (A3) node[below right] {$A_3$} node{$\bullet$};
\draw (A4) node[above right] {$A_4$} node{$\bullet$};
\end{tikzpicture}
\end{center}
