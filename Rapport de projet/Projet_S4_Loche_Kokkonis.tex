...
Pour que notre application soit plus fonctionnelle, nous avons décidé de mettre en place une version PC et Android (voir figures \ref{fig:screenshot_pc} et \ref{fig:screenshot_android}). De cette manière, on permet à l'utilisateur,selon sa préférence, de consulter le correcteur de posture depuis son ordinateur ou son téléphone portable.
...
Pour arriver à ce résultat, nous sommes passés par plusieurs étapes différentes, que nous allons examiner par la suite.

\subsection{La communication en série}
\label{subsec:comm_serie}
La première chose sur laquelle nous avons commencé à travailler peut être considérée comme l’étau de notre application, tout comme la modélisation de la chaise décrite dans la sous-section \ref{subsec:model_chaise}. Il s’agit de la partie \textit{communication}. C'est grâce à ce module qu'on peut finalement recevoir les données des senseurs dans un contexte qui permet de les visualiser et de les rendre plus faciles à interpréter par l'utilisateur.
...
Nous pouvons désormais communiquer avec l'Arduino, en imposant un protocole commun pour les deux appareils. Le module est capable de trouver tous les \textit{ports} de communication en série disponibles sur l'ordinateur (ou le téléphone portable), de les lister et d’accorder à l'utilisateur le lancement d’un flux de communication à travers un de ces ports. Dans un second temps, ce flux permet de recevoir ou même d’envoyer des données : nous rappelons que, même si la fonction principale de l'application est de recevoir des données à partir des senseurs et de les interpréter, on doit être capable d'envoyer une commande afin d’effectuer une \textit{tare}, et réinitialiser ainsi les senseurs.

Finalement, le module \textit{communication en série} comporte un \guillemotleft\ parser \guillemotright , qui permet d'interpréter les données qui arrivent octet par octet depuis l'Arduino, et de les organiser dans un tableau d'entiers de façon que la valeur et l'ID des senseurs soient facilement récupérables. Cette fonctionnalité permet aussi de savoir si on a reçu des données incomplètes, ou même si on n'a rien reçu depuis l'Arduino.
