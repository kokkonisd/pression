...
\resume{Ce projet a comme but la création d'une application de correction de la posture assise d'une personne, dans la forme d'un système embarqué. Le projet a été basé sur un Arduino Uno, qui gère les données envoyés par quatre "load sensors" placées sous les pieds d'une chaise, et qui en suite modifie et envoie ces données en forme "serial" (soit par Bluetooth, soit par USB) à une application basée sur un ordinateur Windows/OSX/Linux ou un appareil Android. Cette application offre une visualisation de la chaise, du barycentre de l'utilisateur et d'une "deadzone" qui est calibrée par l'utilisateur, et qui est utilisée pour détecter si la posture de l'utilisateur est bonne; l'application l'informe visuellement quand ce n'est pas le cas. Le logiciel utilisé est écrit en Java et en C.}



\abstract{This project aims to create an application for correcting one's sitting posture, using both software and hardware. The project was built upon an Arduino Uno that handles the data sent by four load sensors placed under the feet of a chair, and which in turn modifies and sends that data in serial form (either by Bluetooth or by USB) to an application run on a Windows/OSX/Linux computer or an Android device. That device then provides the user with a graphic representation of the chair, the barycenter of the user, as well as a "deadzone" which is calibrated by the user, and is used to detect wether the user's posture is incorrect or not; the application informs the user visually when he is not properly seated. The software is written in Java and C.}
